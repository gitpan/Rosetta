...
tables to SQL::ObjectModel objects, you are free to modify this schema to your
liking. However, it is recommended that you mainly just add columns, such as
for adding user visible text descriptions of each record, rather than changing
or removing the suggested columns.
...
Column List:
	...
	basic_type - str,req - name of the basic data type this is customized from: bin, str, num, bool, datetime
	...
	datetime_calendar - str - when basic_type is 'datetime', what calendar the date and time is in
...
Example Record List:
	...
	|name    |base    |bytes|char|dig|fx|enc|tw|la|pc|tp|u|p|cal|
	|--------|--------|-----|----|---|--|---|--|--|--|--|-|-|---|
	...
	|byte    |num     |    1|    |   | 1|   |  |  |  |  |0|0|   | - a precise int btwn           -128 and           +127
	|short   |num     |    2|    |   | 1|   |  |  |  |  |0|0|   | - a precise int btwn        -32,768 and        +32,767
	|int     |num     |    4|    |   | 1|   |  |  |  |  |0|0|   | - a precise int btwn -2,147,483,648 and +2,147,483,647
	|long    |num     |    8|    |   | 1|   |  |  |  |  |0|0|   | - a precise int btwn        -(2^63) and      +(2^63)-1
	|ubyte   |num     |    1|    |   | 1|   |  |  |  |  |1|0|   | - a precise integer between 0 and           +255
	|ushort  |num     |    2|    |   | 1|   |  |  |  |  |1|0|   | - a precise integer between 0 and        +65,535
	|uint    |num     |    4|    |   | 1|   |  |  |  |  |1|0|   | - a precise integer between 0 and +4,294,967,295
	|ulong   |num     |    8|    |   | 1|   |  |  |  |  |1|0|   | - a precise integer between 0 and      +(2^64)-1
	|float   |num     |    4|    |   | 1|   |  |  |  |  | | |   | - a 32-bit imprecise fractional number
	|double  |num     |    8|    |   | 1|   |  |  |  |  | | |   | - a 64-bit imprecise fractional number
	|dec10p2 |num     |     |    | 10| 0|   |  |  |  |  | |2|   | - a precise 10-digit num with hundredths precision
	|dec255  |num     |     |    |255| 0|   |  |  |  |  | | |   | - a precise 255-digit num with arbitrary precision
...
Column List:
	...
	database_id - int,req - unique identifier for this database
	database_name - str - corresponds to Oracle "Data Source" (for one db instance querying another)

Index List:

	primary - unique - [database_id]
	ak_name - unique - [database_name]
...
Column List:
	...
	database_id - int,req - the database that this namespace is in
	namespace_name - str - the name this namespace is identified by; corresponds to Oracle "Schema"

Index List:

	primary - unique - [namespace_id]
	ak_name - unique - [database_id,namespace_name]
	fk_database - foreign - [database_id]->[database]:[database_id]
