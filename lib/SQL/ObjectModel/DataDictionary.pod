...
All concepts in a SQL::ObjectModel can be represented by a data dictionary.
Such a data dictionary can be a linked in-memory set of objects (or serialized
version thereof), or they can be represented by records in database tables
having specific columns and constraints.  This documentation will attempt to
explain the components of a SQL::ObjectModel data dictionary and how they
relate.  Since SQL::ObjectModel doesn't actually use any database tables
internally, and that it is your application (or other third party modules)
which would map such database tables to SQL::ObjectModel objects, you are free
to modify this schema to your liking. However, it is recommended that you
mainly just add columns, such as for adding user visible text descriptions of
each record, rather than changing or removing the suggested columns.
...
Every table shown here has a single-column primary key so that it is easy to
convert their records into in-memory structs/objects, and their relationships
into normal scalar references each pointing from one struct to another. 
Likewise, the records can be easily converted into XML nodes, where each node
has a single-value unique id, and individual attributes in one XML node can 
refer to such on another.  There are two main kinds of tables.

The tables whose names start with "cct_" have program-defined values and are
not meant to be changed; they are "constant".  Each of those tables represents
an enumeration, and their primary keys are all short strings of lowercased
latin letters and sometimes numbers.  All of these primary keys are on a field
named 'code'.  It is quite possible that these don't need to exist as tables at
all, if the database in use supports column constraints by enumerated list,
rather than just by foreign key.

All of the other tables, which do not start with "cct_", are "normal" tables,
and you can put all of your own records in them.  Each of these tables has a
non-content-descriptive integer for a primary key, which is an intentional
choice; this makes it easy for you to change values which would otherwise be
part of your primary key, without having to worry about updating other copies
to preserve relational integrity; it also saves on memory usage.  All of these
primary keys are on a field named 'id'.
...
=head2 Applications, Commands and Results

	cct_command_type   command_var (bind variables passed from application)
	      |                \|/
	     /|\ |---------------
	   command=>---application
...
These tables correspond to basic concepts that are hard-coded into SQL::ObjectModel and
...
to code them yourself.  The other SQL::ObjectModel classes use the Codes class
...
Column List:
	...
	size_is_digits - bool,req - true when can customize type to a size measured in digits
	typ_min_size - uint - suggested typical minimum size to customize to (in any above unit)
	typ_max_size - uint - suggested typical maximum size to customize to (in any above unit)
...
Record List:
	...
	|code    |un|by|ch|di|min|       max|
	|--------|--|--|--|--|---|----------|
	|bin     |0 |1 |0 |0 |  1|4294967295| - stores anything as binary data
	|str     |0 |1 |1 |0 |  1|4294967295| - stores character data
	|num     |0 |1 |1 |1 |  1|       255| - a number, either integral or fractional
...
Column List:
	...
	is_var_len_enc - bool,req - true when char encoding is variable length, false when fixed length
	min_bytes_per_char - int,req - minimum num bytes used to store each character; typically 1
	max_bytes_per_char - int,req - maximum bytes per char, same as minimum when fixed length
...
Record List:
	...
	|cod|vle|nbpc|xbpc|lat|
	|---|---|----|----|---|
	|u8 |1  |   1|   6|1  | - UTF-8 (always byte-order; no big/little endian; typ 1 byte or 3 byte)
	|u16|1  |   2|  ?4|1  | - UTF-16 (has big/little endian versions)
	|u32|0  |   4|   4|1  | - UTF-32 (has big/little endian versions)
	|asc|0  |   1|   1|1  | - ascii
	|ebs|0  |   1|   1|1  | - ebsdic
...
This table enumerates the basic types that a view can be, since SQL::ObjectModel "views"
...
Column List:
	...
	data_type - enum - basic data type that function returns
...
Record List:
	...
	|to_num |1 | 1|  |0 | 1|1 |1 |1 |0 |num     | - cast arg as numeric (might take more args)
	|to_int |1 | 1|  |0 | 1|1 |1 |1 |0 |num     | - cast arg as integer (might take more args)
	...
	|add    |1 | 2| 1|1 |  |1 |1 |1 |0 |num     | - result of adding all args as numbers
	|sub    |1 | 2| 1|1 |  |1 |1 |1 |0 |num     | - result of subtracting all subsequent args from first
	|mul    |1 | 2| 1|1 |  |1 |1 |1 |0 |num     | - result of multiplying all arguments
	|div    |1 | 2|  |0 | 2|1 |1 |1 |0 |num     | - result of dividing first argument by second
	|divi   |1 | 2|  |0 | 2|1 |1 |1 |0 |num     | - integral division of first arg by second
	|mod    |1 | 2|  |0 | 2|1 |1 |1 |0 |num     | - modulus of integral division of first arg by second
	|round  |1 | 1| 1|0 | 2|1 |1 |1 |0 |num     | - rounds first arg to N dec places; N is second arg or 0
	|exp    |1 | 2| 1|0 | 2|1 |1 |1 |0 |num     | - raises first arg to the power of the second
	|log    |1 | 2| 1|0 | 2|1 |1 |1 |0 |num     | - logarithm of the first arg on the base of second
	|min    |1 | 1| 1|1 |  |1 |1 |1 |0 |num     | - minimum arg value of all arguments
	|max    |1 | 1| 1|1 |  |1 |1 |1 |0 |num     | - maximum arg value of all arguments
	|avg    |1 | 1| 1|1 |  |1 |1 |1 |0 |num     | - average value of all arguments
	|sconcat|1 | 1| 1|1 |  |1 |1 |1 |0 |str     | - str concat of all arguments
	|slength|1 | 1|  |0 | 1|1 |1 |1 |0 |num     | - length of input string in characters
	|sindex |1 | 2| 1|0 | 3|1 |1 |1 |0 |num     | - pos in arg 1 of arg 2 if present, start at arg 3
	...
	|gcount |0 | 0|  |0 | 0|1 |0 |0 |1 |num     | - count of rows a view/cursor can see
	|gmin   |1 | 1|  |0 | 1|1 |0 |0 |1 |num     | - minimum of values in all records in one view col
	|gmax   |1 | 1|  |0 | 1|1 |0 |0 |1 |num     | - maximum of values in all records in one view col
	|gsum   |1 | 1|  |0 | 1|1 |0 |0 |1 |num     | - sum of values in all records in one view col
	|gavg   |1 | 1|  |0 | 1|1 |0 |0 |1 |num     | - average of values in all records in one view col
	...
	|crownum|0 | 0|  |0 | 0|1 |0 |0 |0 |num     | - row number of returned row, before sorting (eg:Oracle)
	|clevel |0 | 0|  |0 | 0|1 |0 |0 |0 |num     | - when using st-wi-cn-by, what hierarchy level row is in (eg:Oracle)
...
This table enumerates the basic types that a blocks can be, since SQL::ObjectModel
...
This table enumerates the basic types of SQL::ObjectModel Commands that an application 
...
This table enumerates the basic types of Results that SQL::ObjectModel can return from executing a 
...
would correspond to a basic type that is recognized in code by SQL::ObjectModel, and
...
to anything in the SQL::ObjectModel code.
...
Column List:
	...
	id - int,req - unique identifier for this data type
	name - str,req - name of the user customized data type
	basic_type - enum,req - name of the basic data type this is customized from: bin, str, num, bool, datetime
	size_in_bytes - int - maximum size in bytes data can be
	size_in_chars - int - maximum size in chars data can be
	size_in_digits - int - maximum size in digits data can be (aka num_scale)
	store_fixed - bool - when true then try to reserve max size on disk even if not all used
	str_encoding - enum - which character encoding to use when data type is a string
	str_trim_white - bool - when true then trim lead/trailing whitespace on save into field/var
	str_latin_case - enum - when str has latin chars do we upper/lowercase them on save
	...
	num_unsigned - bool - when basic_type is 'num', true means only >= 0 may be stored
	num_precision - int - when basic_type is 'num', num digits of fractional precision we allowed
	datetime_calendar - enum - when basic_type is 'datetime', what calendar the date and time is in

Undecided Possible Extra Columns:

	num_min_val - int - when type is numerical, what the allowed minimum value is
	num_max_val - int - when type is numerical, what the allowed maximum value is
	datetime_min_val - datetime - when type is datetime, what the allowed earliest value is
	datetime_max_val - datetime - when type is datetime, what the allowed latest value is

Index List:

	primary - unique - [id]
	ak_name - unique - [name]
	fk_basic_type - foreign - [basic_type]->[cct_basic_data_type]:[code]
	fk_str_encoding - foreign - [str_encoding]->[cct_str_enc]:[code]
	fk_str_latin_case - foreign - [str_latin_case]->[cct_str_latin_case]:[code]
	fk_datetime_calendar - foreign - [datetime_calendar]->[cct_datetime_calendar]:[code]

Example Record List:

	|id|name    |base    |bytes|char|dig|fx|enc|tw|la|pc|tp|u|p|cal|
	|--|--------|--------|-----|----|---|--|---|--|--|--|--|-|-|---|
	| 1|bin1k   |bin     | 1000|    |   |  |   |  |  |  |  | | |   | - a hunk of binary data of maximum 1000 bytes
	| 2|bin32k  |bin     |32000|    |   |  |   |  |  |  |  | | |   | - a hunk of binary data of maximum 32000 bytes
	| 3|str4    |str     |     |   4|   | 1|asc| 1|uc|20|1 | | |   | - a fix-w 4 char ascii str, uppercased, spc-pad
	| 4|str10   |str     |     |  10|   | 1|asc| 1|pr|20|1 | | |   | - a fix-w 10 char ascii str, case-preserve, spc-pad
	| 5|str30   |str     |     |  30|   |  |asc| 1|  |  |  | | |   | - a var-w 30 char ascii str, case-preserving
	| 6|str2k   |str     |     |2000|   |  |u16|  |  |  |  | | |   | - a var-w 2000 char unicode str, case-preserving
	| 7|byte    |num     |    1|    |   |  |   |  |  |  |  | |0|   | - a precise int btwn           -128 and           +127
	| 8|short   |num     |    2|    |   |  |   |  |  |  |  | |0|   | - a precise int btwn        -32,768 and        +32,767
	| 9|int     |num     |    4|    |   |  |   |  |  |  |  | |0|   | - a precise int btwn -2,147,483,648 and +2,147,483,647
	|10|long    |num     |    8|    |   |  |   |  |  |  |  | |0|   | - a precise int btwn        -(2^63) and      +(2^63)-1
	|11|ubyte   |num     |    1|    |   |  |   |  |  |  |  |1|0|   | - a precise integer between 0 and           +255
	|12|ushort  |num     |    2|    |   |  |   |  |  |  |  |1|0|   | - a precise integer between 0 and        +65,535
	|13|uint    |num     |    4|    |   |  |   |  |  |  |  |1|0|   | - a precise integer between 0 and +4,294,967,295
	|14|ulong   |num     |    8|    |   |  |   |  |  |  |  |1|0|   | - a precise integer between 0 and      +(2^64)-1
	|15|float   |num     |    4|    |   |  |   |  |  |  |  | | |   | - a 32-bit imprecise fractional number
	|16|double  |num     |    8|    |   |  |   |  |  |  |  | | |   | - a 64-bit imprecise fractional number
	|17|dec10p2 |num     |     |    | 10|  |   |  |  |  |  | |2|   | - a precise 10-digit num with hundredths precision
	|18|dec255  |num     |     |    |255|  |   |  |  |  |  | | |   | - a precise 255-digit num with arbitrary precision
	|19|boolean |bool    |     |    |   |  |   |  |  |  |  | | |   | - a boolean; only 2 values of false or true
	|20|datetime|datetime|     |    |   |  |   |  |  |  |  | | |abs| - a date or time or both, non-calendar specific
	|21|dtchines|datetime|     |    |   |  |   |  |  |  |  | | |chi| - a date or time or both, in the Chinese calendar
	|22|str1    |str     |     |   1|   |  |   |  |  |  |  | | |   | - a 1-char string
	|23|str20   |str     |     |  20|   |  |   |  |  |  |  | | |   | - a 20-char string
	|24|str100  |str     |     | 100|   |  |   |  |  |  |  | | |   | - a 100-char string
	|25|str250  |str     |     | 250|   |  |   |  |  |  |  | | |   | - a 250-char string
	|26|entitynm|str     |     |  30|   |  |   |  |  |  |  | | |   | - a short string suitable for naming things
	|27|generic |str     |     | 250|   |  |   |  |  |  |  | | |   | - a longer string that most kinds of data will fit in

=head1 DATABASES AND NAME SPACES

SQL::ObjectModel data dictionaries can be and are used to describe both things
...
normal 'select' SQL.  There are two root tables in SQL::ObjectModel data
...
Column List:
	...
	id - int,req - unique identifier for this database
	name - str - corresponds to Oracle "Data Source" (for one db instance querying another)
...
Example Record List:
	...
	|id|name|
	|--|----|
	| 1|    | - a default database, usually just used if it is the only database
	| 2|site| - a multi-namespace genealogy database that you manage
	| 3|dist| - a similar database that someone else manages
...
SQL::ObjectModel will let you define your schema objects (tables, views, procedures,
etc) as if multiple namespaces were supported natively, and if you are
deploying to a database that doesn't, then SQL::ObjectModel can put a common name prefix
...
Column List:
	...
	id - int,req - unique identifier for this namespace
	database - int,req - the database that this namespace is in
	name - str - the name this namespace is identified by; corresponds to Oracle "Schema"

Index List:

	primary - unique - [id]
	ak_name - unique - [database,name]
	fk_database - foreign - [database]->[database]:[id]

Example Record List:

	|id|db|name|
	|--|--|----|
	| 1| 1|    | - a default namespace, usually just used if it is the only namespace
	| 2| 2|gene| - store all the normal app data that constitutes a genealogy database
	| 3| 2|dd  | - store a data dictionary describing an application for editing gene data
	| 4| 3|gene| - in case we want to access genealogy data collected by someone else
...
Column List:
	...
	id - int,req - unique identifier for this table
	namespace - int,req - the namespace that this table is in
	name - str,req - the name this table is identified by
	order - int,req - to explicitely define what order tables are declared in
...
Index List:
	...
	primary - unique - [id]
	ak_name - unique - [namespace,name]
	ak_order - unique - [namespace,order]
	ak_public - unique - [public_syn]
	fk_namespace - foreign - [namespace]->[namespace]:[id]
...
Column List:
	...
	id - int,req - unique identifier for this table column
	table - int,req - unique identifier for this table
	name - str,req - unique identifier for column within table
	order - int,req - what order columns are declared in (and by default returned in)
	data_type - int,req - the user-customized data type for this column
...
Index List:
	...
	primary - unique - [id]
	ak_name - unique - [table,name]
	ak_order - unique - [table,order]
	fk_table - foreign - [table]->[table]:[id]
	fk_data_type - foreign - [data_type]->[data_type]:[id]

Undecided Possible Extra Indexes:

	fk_sequence - foreign - [default_seq]->[sequence]:[id]

Example Record List:

	|id|tb|name         |ord|ty|req|def|inc|
	|--|--|-------------|---|--|---|---|---|
	| 1| 1|user_id      |  1| 9|1  |1  |1  | - unique internal id for user, not changeable
	| 2| 1|login_name   |  2|23|1  |   |   | - who user authenticates into app as
	| 3| 1|login_pass   |  3|23|1  |   |   | - also used with authentication
	| 4| 1|private_name |  4|24|1  |   |   | - real name that app service provider can know
	| 5| 1|private_email|  5|24|1  |   |   | - email that service provider can contact user with
	| 6| 1|may_login    |  6|19|1  |   |   | - true when user may use app at all
	| 7| 1|max_sessions |  7| 7|1  |3  |   | - max concurrent app logins user may have
	| 8| 2|user_id      |  1| 9|1  |   |   | - unique internal id for user, not changeable
	| 9| 2|public_name  |  2|25|1  |   |   | - public name or pen name or public alias
	|10| 2|public_email |  3|25|0  |   |   | - email address the public may use for contact
	|11| 2|web_url      |  4|25|0  |   |   | - personal/business website address of user
	|12| 2|contact_net  |  5|25|0  |   |   | - info like ICQ number or other internet handles
	|13| 2|contact_phy  |  6|25|0  |   |   | - physical address or city or telephone number
	|14| 2|bio          |  7|25|0  |   |   | - brief description of the person
	|15| 2|plan         |  8|25|0  |   |   | - what the person plans to do next
	|16| 2|comments     |  9|25|0  |   |   | - other comments
	|17| 3|user_id      |  1| 9|1  |   |   | - unique internal id for user, not changeable
	|18| 3|pref_name    |  2|26|1  |   |   | - app-specific preference name
	|19| 3|pref_value   |  3|27|0  |   |   | - preference value for this user
	|20| 4|person_id    |  1| 9|1  |1  |1  | - unique actual id for person, not changeable
	|21| 4|alternate_id |  2|23|0  |   |   | - user's own way of ident people, if wanted
	|22| 4|name         |  3|24|1  |   |   | - actual name of the person they are called by
	|23| 4|sex          |  4|22|0  |   |   | - sex of the person, if known
	|24| 4|father_id    |  5| 9|0  |   |   | - person_id of this person's father, if known
	|25| 4|mother_id    |  6| 9|0  |   |   | - person_id of this person's mother, if known
...
Column List:
	...
	id - int,req - unique identifier for this table index
	table - int,req - unique identifier for this table
	name - str,req - unique identifier for index within table
	order - int,req - what order indexes are declared in
	ind_type - enum,req - what type of index this is (eg: unique, foreign)
	f_table - int - when the index type is a foreign key, this is the constraining table
...
Index List:
	...
	fk_f_table - foreign - [f_table]->[table]:[id]

Example Record List:

	|id|tb|name            |ord|type   |ftbl|
	|--|--|----------------|---|-------|----|
	| 1| 1|primary         |  1|unique |    | - ensure all [user_id] are unique
	| 2| 1|ak_login_name   |  2|unique |    | - ensure all [login_name] are unique
	| 3| 1|ak_private_email|  3|unique |    | - ensure all [private_email] are unique
	| 4| 2|primary         |  1|unique |    | - ensure all [user_id] are unique
	| 5| 2|ak_public_name  |  2|unique |    | - ensure all [public_name] are unique
	| 6| 2|fk_user         |  3|foreign|   1| - ensure user_profile matches user_auth
	| 7| 3|primary         |  1|unique |    | - ensure all [user_id,pref_name] are unique
	| 8| 3|fk_user         |  2|foreign|   1| - ensure user_pref matches user_auth
	| 9| 4|primary         |  1|unique |    | - ensure all [person_id] are unique
	|10| 4|ak_alternate_id |  2|unique |    | - ensure all [alternate_id] are unique
	|11| 4|fk_father       |  3|foreign|   4| - link child to father
	|12| 4|fk_mother       |  4|foreign|   4| - link child to mother
...
Column List:
	...
	id - int,req - unique identifier for this table index column
	table_ind - int,req - unique identifier for index within table
	table_col - int,req - unique identifier for column within table
	order - int,req - order of the columns within the index
	f_table_col - int - when the index type is a foreign key, this is the constraining column
...
Index List:
	...
	primary - unique - [id]
	ak_table_col - unique - [table_ind,table_col]
	ak_order - unique - [table_ind,order]
	fk_table_ind - foreign - [table_ind,table]->[table_ind]:[id,table]
	fk_table_col - foreign - [table_col,table]->[table_col]:[id,table]
	fk_f_table_col - foreign - [f_table_col]->[table_col]:[id]

Example Record List:

	|id|ind|col|ord|tbl|ftbl|fcol|
	|--|---|---|---|---|----|----|
	| 1|  1|  1|  1|  1|    |    | - ensure all [user_id] are unique
	| 2|  2|  2|  1|  1|    |    | - ensure all [login_name] are unique
	| 3|  3|  5|  1|  1|    |    | - ensure all [private_email] are unique
	| 4|  4|  8|  1|  2|    |    | - ensure all [user_id] are unique
	| 5|  5|  9|  1|  2|    |    | - ensure all [login_name] are unique
	| 6|  6|  8|  1|  2|   1|   1| - ensure user_profile matches user_auth
	| 7|  7| 17|  1|  3|    |    | - ensure all [user_id,pref_name] are unique
	| 8|  7| 18|  2|  3|    |    | - ensure all [user_id,pref_name] are unique
	| 9|  8| 17|  1|  3|   1|   1| - ensure user_pref matches user_auth
	|10|  9| 20|  1|  4|    |    | - ensure all [person_id] are unique
	|11| 10| 21|  1|  4|    |    | - ensure all [alternate_id] are unique
	|12| 11| 24|  1|  4|   4|  20| - link child to father
	|13| 12| 25|  1|  4|   4|  20| - link child to mother
...
Column List:
	...
	id - int,req - unique identifier for this trigger
	table - int,req - unique identifier for the table to watch
	block - int,req - unique identifier for the block to execute, only 1 trig can use a block
...
Index List:
	...
	primary - unique - [id]
	ak_block - unique - [block]
	fk_table - foreign - [table]->[table]:[id]
	fk_block - foreign - [block]->[block]:[id]
...
describe how data is stored, but mainly just how it is retrieved.  SQL::ObjectModel
...
primary keys for rows being updated), SQL::ObjectModel allows you to store a hint with a
...
Column List:
	...
	id - int,req - unique identifier for this view
	view_type - enum,req - what kind of view this is; eg: object, caller, cursor, inside
	namespace - int - the namespace that this view is in, if any
	name - str - the name this view is identified by, if any
	order - int - to explicitely define what order views are declared in, if named
...
Undecided Possible Extra Columns:
	...
	maxr_block_var - int - if paging, get max rows from this containing block variable
	maxr_command_var - int - if paging, get max rows from this caller bind variable
	skipr_lit_val - int - if paging, this literal value says rows to skip before returning any
	skipr_block_var - int - if paging, get skip rows from this containing block variable
	skipr_command_var - int - if paging, get skip rows from this caller bind variable
...
Index List:
	...
	fk_namespace - foreign - [namespace]->[namespace]:[id]
	fk_match_table - foreign - [match_table]->[table]:[id]

Undecided Possible Extra Indexes:

	fk_maxr_block_var - foreign - [maxr_block_var]->[block_var]:[id]
	fk_maxr_command_var - foreign - [maxr_command_var]->[command_var]:[id]
	fk_skipr_block_var - foreign - [skipr_block_var]->[block_var]:[id]
	fk_skipr_command_var - foreign - [skipr_command_var]->[command_var]:[id]
...
Column List:
	...
	id - int,req - unique identifier for this view column
	view - int,req - unique identifier for this view
	name - str,req - unique identifier for column within view
	order - int,req - what order columns are declared in (and by default returned in)
	data_type - int,req - the user-customized data type for this column
...
Index List:
	...
	primary - unique - [id]
	ak_name - unique - [view,name]
	ak_order - unique - [view,order]
	fk_view - foreign - [view]->[view]:[id]
	fk_data_type - foreign - [data_type]->[data_type]:[id]

Example Record List:

	|id|vw|col_name     |ord|ty|so|
	|--|--|-------------|---|--|--|
	| 1| 1|user_id      |  1| 9|  | - unique internal id for user, not changeable
	| 2| 1|login_name   |  2|23| 1| - who user authenticates into app as
	| 3| 1|login_pass   |  3|23|  | - also used with authentication
	| 4| 1|private_name |  4|24|  | - real name that app service provider can know
	| 5| 1|private_email|  5|24|  | - email that service provider can contact user with
	| 6| 1|may_login    |  6|19|  | - true when user may use app at all
	| 7| 1|max_sessions |  7| 7|  | - max concurrent app logins user may have
	| 8| 1|public_name  |  8|25|  | - public name or pen name or public alias
	| 9| 1|public_email |  9|25|  | - email address the public may use for contact
	|10| 1|web_url      | 10|25|  | - personal/business website address of user
	|11| 1|contact_net  | 11|25|  | - info like ICQ number or other internet handles
	|12| 1|contact_phy  | 12|25|  | - physical address or city or telephone number
	|13| 1|bio          | 13|25|  | - brief description of the person
	|14| 1|plan         | 14|25|  | - what the person plans to do next
	|15| 1|comments     | 15|25|  | - other comments
	|16| 2|self_id      |  1| 9|  | - unique actual id for person, not changeable
	|17| 2|self_name    |  2|24| 1| - actual name of the person they are called by
	|18| 2|father_id    |  3| 9|  | - person_id of this person's father, if known
	|19| 2|father_name  |  4|24| 2| - actual name of the person's father
	|20| 2|mother_id    |  5| 9|  | - person_id of this person's mother, if known
	|21| 2|mother_name  |  6|24| 3| - actual name of the person's mother
	|22| 3|theme_name   |  1|27| 2| - name of a user theme
	|23| 3|theme_count  |  2| 9| 1| - count of users that use this theme
...
Column List:
	...
	id - int,req - unique identifier for this view rowset
	view - int,req - unique identifier for this view
	p_rowset - int - parent rowset of this one, if any; empty if this is the root rowset
	p_rowset_order - int,req - order of this rowset among its siblings; 1 if no siblings
	c_merge_type - enum - how to merge any child rowsets of this one together, if any

Index List:

	primary - unique - [id]
	ak_order - unique - [p_rowset,p_rowset_order]
	fk_view - foreign - [view]->[view]:[id]
...
Example Record List:
	...
	|--|--|---|---|---|
	| 1| 1|   |  1|   | - user
	| 2| 2|   |  1|   | - person_with_parents
	| 3| 3|   |  1|   | - user_theme

=head2 view_src

Each record in this table uniquely corresponds to a source table or view that
is used by this view (one copy per view rowset if applicable).  Specifically it
applies only to the "from" clause in the generated SQL, where applicable.  A
table or view may be used for multiple sources, each of which would be a unique
alias for it.

Column List:

	id - int,req - unique identifier for this view source
	rowset - int,req - unique identifier for rowset within view
	name - str,req - unique "alias" identifier for source within view rowset
	order - int,req - what order sources are declared in (and by default joined)
...
Index List:
	...
	primary - unique - [id]
	ak_name - unique - [rowset,name]
	ak_order - unique - [rowset,order]
	fk_rowset - foreign - [rowset]->[view_rowset]:[id]
	fk_match_table - foreign - [match_table]->[table]:[id]
	fk_match_view - foreign - [match_view]->[view]:[id]

Example Record List:

	|id|rs|name        |ord|mtb|mvw|
	|--|--|------------|---|---|---|
	| 1| 1|user_auth   |  1|  1|   | - match "user_auth" table
	| 2| 1|user_profile|  2|  2|   | - match "user_profile" table
	| 3| 2|self        |  1|  4|   | - match "person" table
	| 4| 2|father      |  2|  4|   | - match "person" table
	| 5| 2|mother      |  3|  4|   | - match "person" table
	| 6| 3|user_pref   |  1|  3|   | - match "user_pref" table
...
Note that with databases that require all used columns to be populated in order
to enforce a unique key or foreign key constraint, a trigger or other mechanism
may be needed for some of the below items.

Column List:

	id - int,req - unique identifier for this view source column
	src - int,req - which view source this column is associated with
	match_table_col - int - which column from the source table is represented, if applicable
	match_view_col - int - which column from the source view is represented, if applicable

	rowset - int,req - unique identifier for rowset within view (for fk to col table)
	match_table - int - if set, the source is a table (and match_view must not be set)
	match_view - int - if set, the source is another view (and match_table must not be set)
...
Index List:
	...
	primary - unique - [id]
	fk_view_src - foreign - [src,rowset]->[view_src]:[id,rowset]
	fk_match_table - foreign - [src,match_table]->[view_src]:[id,match_table]
	fk_match_view - foreign - [src,match_view]->[view_src]:[id,match_view]
	fk_match_table_col - foreign - [match_table,match_table_col]->[table_col]:[id,table]
	fk_match_view_col - foreign - [match_view,match_view_col]->[view_col]:[id,view]

Example Record List:

	|id|vs|mtb|mtc|mvw|mvc|ri|
	|--|--|---|---|---|---|--|
	| 1| 1|  1|  1|   |   | 1| - "user_auth"."user_id"
	| 2| 1|  1|  2|   |   | 1| - "user_auth"."login_name"
	| 3| 1|  1|  3|   |   | 1| - "user_auth"."login_pass"
	| 4| 1|  1|  4|   |   | 1| - "user_auth"."private_name"
	| 5| 1|  1|  5|   |   | 1| - "user_auth"."private_email"
	| 6| 1|  1|  6|   |   | 1| - "user_auth"."may_login"
	| 7| 1|  1|  7|   |   | 1| - "user_auth"."max_sessions"
	| 8| 2|  2|  8|   |   | 1| - "user_profile"."user_id"
	| 9| 2|  2|  9|   |   | 1| - "user_profile"."public_name"
	|10| 2|  2| 10|   |   | 1| - "user_profile"."public_email"
	|11| 2|  2| 11|   |   | 1| - "user_profile"."web_url"
	|12| 2|  2| 12|   |   | 1| - "user_profile"."contact_net"
	|13| 2|  2| 13|   |   | 1| - "user_profile"."contact_phy"
	|14| 2|  2| 14|   |   | 1| - "user_profile"."bio"
	|15| 2|  2| 15|   |   | 1| - "user_profile"."plan"
	|16| 2|  2| 16|   |   | 1| - "user_profile"."comments"
	|17| 3|  4| 20|   |   | 2| - "self"."person_id"
	|18| 3|  4| 22|   |   | 2| - "self"."name"
	|19| 4|  4| 20|   |   | 2| - "father"."person_id"
	|20| 4|  4| 22|   |   | 2| - "father"."name"
	|21| 5|  4| 20|   |   | 2| - "mother"."person_id"
	|22| 5|  4| 22|   |   | 2| - "mother"."name"
	|23| 6|  3| 18|   |   | 3| - "user_pref"."pref_name"
	|24| 6|  3| 19|   |   | 3| - "user_pref"."pref_value"
	|25| 3|  4| 24|   |   | 2| - "self"."father_id"
	|26| 3|  4| 25|   |   | 2| - "self"."mother_id"
...
Column List:
	...
	id - int,req - unique identifier for this view join
	rowset - int,req - unique identifier for rowset within view
	lhs_src - int,req - which view source is on the left side of the join
	rhs_src - int,req - which view source is on the right side of the join
	join_type - enum,req - what kind of table/view join is being done

Index List:

	primary - unique - [id]
	ak_join - unique - [lhs_src,rhs_src]
	ak_limit_one - unique - [rhs_src]
	fk_lhs_src - foreign - [lhs_src,rowset]->[view_src]:[id,rowset]
	fk_rhs_src - foreign - [rhs_src,rowset]->[view_src]:[id,rowset]
...
Column List:
	...
	id - int,req - unique identifier for this view join column
	join - int,req - which view join this column is associated with
	lhs_src_col - int,req - which column from the left source is being matched on
	rhs_src_col - int,req - which column from the right source is being matched on
...
Index List:
	...
	primary - unique - [id]
	ak_join - unique - [lhs_src,rhs_src,lhs_src_col]
	fk_join - foreign - [join,lhs_src,rhs_src]->[view_join]:[id,lhs_src,rhs_src]
	fk_lhs_src_col - foreign - [lhs_src_col,lhs_src]->[view_src_col]:[id,src]
	fk_rhs_src_col - foreign - [rhs_src_col,rhs_src]->[view_src_col]:[id,src]

Example Record List:

	|id|pa|lhs|lhc|rhs|rhc|
	|--|--|---|---|---|---|
	| 1| 1|  1|  1|  2|  8| - on "user_profile"."user_id" = "user_auth"."user_id"
	| 2| 2|  3| 25|  4| 19| - on "father"."person_id" = "self"."father_id"
	| 3| 3|  3| 26|  5| 21| - on "mother"."person_id" = "self"."mother_id"
...
Column List:
	...
	id - int,req - unique identifier for this view hierarchy
	rowset - int,req - unique identifier for rowset within view
	start_src_col - int,req - which source column is checked against to determine the root of the hierarchy
	start_lit_val - str - if start_src_col is being compared to a literal, then store the literal value here
	start_block_var - int - if start_src_col is being compared to a containing block variable
	start_command_var - int - if start_src_col is being compared to a caller bind variable
	conn_src_col - int,req - which source column contains a record's own identifier against which children match
	p_conn_src_col - int,req - which source column contains the value of a parent identifier

	src - int,req - which view source is being hierarchically joined to itself

Index List:

	primary - unique - [id]
	ak_rowset - unique - [rowset]
	fk_rowset - foreign - [rowset]->[view_rowset]:[id]
	fk_src - foreign - [src,rowset]->[view_src]:[id,rowset]
	fk_start_src_col - foreign - [start_src_col,src]->[view_src_col]:[id,src]
	fk_start_block_var - foreign - [start_block_var]->[block_var]:[id]
	fk_start_command_var - foreign - [start_command_var]->[command_var]:[id]
	fk_conn_src_col - foreign - [conn_src_col,src]->[view_src_col]:[id,src]
	fk_p_conn_src_col - foreign - [p_conn_src_col,src]->[view_src_col]:[id,src]

Note that the current structure only allows queries like "... start with
[start_src_col] = [start_[lit_val/[block/command]_var]] connect by prior
[conn_src_col] = [p_conn_src_col] ...", but this probably isn't adequate for
all uses, and will be improved.  We may eliminate this table in favor of
view_part_def.
...
Column List:
	...
	id - int,req - unique identifier for this view column def sub-expression
	view - int,req - unique identifier for this view
	view_col - int,req - unique identifier for interface/output column within view
	rowset - int,req - unique identifier for rowset within view
	p_expr - int - parent expression of this one, if any; empty if this is the root expression
	p_expr_order - int,req - order of this expression among its siblings; 1 if no siblings
	expr_type - enum,req - what type of expression this is; eg: literal, variable, column, view, function
	lit_val - str - if expr_type is 'lit', then store the literal value here
	block_var - int - if expr_type is 'var', then use this containing block variable
	command_var - int - if expr_type is 'var', then use this caller bind variable
	src_col - int - if expr_type is 'col', then use this source table/view column
	f_view - int - if expr_type is 'view', then put the view/subquery id here
	sfunc - enum - if expr_type is 'sfunc', then put the standard built-in function name/code here
	ufunc - int - if expr_type is 'ufunc', then put the user-defined stored function (block) id here

Index List:

	primary - unique - [id]
	ak_order - unique - [view_col,rowset,p_expr,p_expr_order]
	fk_view_col - foreign - [view_col,view]->[view_col]:[id,view]
	fk_rowset - foreign - [rowset,view]->[view_rowset]:[id,view]
	fk_expr_type - foreign - [c_expr_type]->[cct_basic_expr_type]:[code]
	fk_block_var - foreign - [block_var]->[block_var]:[id]
	fk_command_var - foreign - [command_var]->[command_var]:[id]
	fk_src_col - foreign - [src_col,rowset]->[view_src_col]:[id,rowset]
	fk_f_view - foreign - [f_view]->[view]:[id]
	fk_sfunc - foreign - [sfunc]->[cct_standard_func]:[code]
	fk_ufunc - foreign - [ufunc]->[block]:[id]

Example Record List:

	|id|vw|ci|ri|pe|peo|type |lit|blv|cmv|src|fvw|sf    |uf|
	|--|--|--|--|--|---|-----|---|---|---|---|---|------|--|
	| 1| 1| 1| 1|  |  1|col  |   |   |   |  1|   |      |  |
	| 2| 1| 2| 1|  |  1|col  |   |   |   |  2|   |      |  |
	| 3| 1| 3| 1|  |  1|col  |   |   |   |  3|   |      |  |
	...
	|21| 2|21| 2|  |  1|col  |   |   |   | 22|   |      |  |
	|22| 3|22| 3|  |  1|col  |   |   |   | 24|   |      |  |
	|23| 3|23| 3|  |  1|sfunc|   |   |   |   |   |gcount|  |
	|24| 3|23| 3|23|  1|col  |   |   |   | 24|   |      |  |
...
Column List:
	...
	id - int,req - unique identifier for this view part def sub-expression
	rowset - int,req - unique identifier for rowset within view
	view_part - enum,req - what "part" of a view definition this is defining
	p_expr - int - parent expression of this one, if any; empty if this is the root expression
	p_expr_order - int,req - order of this expression among its siblings; 1 if no siblings
	expr_type - enum,req - what type of expression this is; eg: literal, variable, column, view, function
	lit_val - str - if expr_type is 'lit', then store the literal value here
...
Index List:
	...
	primary - unique - [id]
	ak_order - unique - [rowset,view_part,p_expr,p_expr_order]
	fk_rowset - foreign - [rowset]->[view_rowset]:[id]
...
Example Record List:
	...
	|id|ri|part |pe|peo|type |lit  |blv|cmv     |src|fvw|sf    |uf|
	|--|--|-----|--|---|-----|-----|---|--------|---|---|------|--|
	| 1| 1|where|  |  1|sfunc|     |   |        |   |   |eq    |  | - standard function of "eq(a,b)"
	| 2| 1|where| 1|  1|col  |     |   |        |  1|   |      |  | - match column "user_auth"."user_id"
	| 3| 1|where| 1|  2|var  |     |   |curr_uid|   |   |      |  | - value of bind var "curr_uid"
	| 4| 2|where|  |  1|sfunc|     |   |        |   |   |and   |  | - standard function of "and(a,b)"
	| 5| 2|where| 4|  1|sfunc|     |   |        |   |   |like  |  | - standard function of "like(a,b)"
	| 6| 2|where| 5|  1|col  |     |   |        | 20|   |      |  | - match column "father"."name"
	| 7| 2|where| 5|  2|var  |     |   |srchw_fa|   |   |      |  | - value of bind var "srchw_fa"
	| 8| 2|where| 4|  1|sfunc|     |   |        |   |   |like  |  | - standard function of "like(a,b)"
	| 9| 2|where| 8|  1|col  |     |   |        | 22|   |      |  | - match column "mother"."name"
	|10| 2|where| 8|  2|var  |     |   |srchw_mo|   |   |      |  | - value of bind var "srchw_mo"
	|11| 3|where|  |  1|sfunc|     |   |        |   |   |eq    |  | - standard function of "eq(a,b)"
	|12| 3|where|11|  1|col  |     |   |        | 23|   |      |  | - match column "user_pref"."pref_name"
	|13| 3|where|11|  2|lit  |theme|   |        |   |   |      |  | - match literal value of "theme"
	|14| 3|group|  |  1|col  |     |   |        | 24|   |      |  | - group by "user_pref"."pref_value"
	|15| 3|havin|  |  1|sfunc|     |   |        |   |   |gt    |  | - standard function of "gt(a,b)"
	|16| 3|havin|15|  1|sfunc|     |   |        |   |   |gcount|  | - standard function of "gcount(col)"
	|17| 3|havin|15|  2|lit  |    1|   |        |   |   |      |  | - match literal value of "theme"
...
Column List:
	...
	id - int,req - unique identifier for this block
	block_type - enum,req - what kind of block this is; eg: object, caller, cursor, inside
	namespace - int - the namespace that this block is in, if any
	name - str - the name this block is identified by, if any
	order - int - to explicitely define what order blocks are declared in, if named
	public_syn - str - a name for the block outside of any namespace, if named; Oracle public synonym

	p_block - int - parent block of this one, if any; empty if this is the root block
...
Column List:
	...
	id - int,req - unique identifier for this block var
	block - int,req - what block this variable is in
	name - str,req - unique identifier for variable within view
	order - int,req - what order variables are declared in
	var_type - enum,req - what basic variable type the variable is
	is_argument - bool,req - true if this variable is a block argument (needs fleshing out)
	data_type - int - if the var type is 'scalar', this is its user customized data type
	init_lit_val - str - if var type is 'scalar', an initial value for var can go here
	c_view - int - if var type is 'cursor', then put the view defining it here
...
Index List:
	...
	primary - unique - [id]
	ak_name - unique - [block,name]
	ak_order - unique - [block,order]
	fk_block - foreign - [block]->[block]:[id]
	fk_var_type - foreign - [var_type]->[cct_basic_var_type]:[code]
	fk_data_type - foreign - [data_type]->[data_type]:[id]
	fk_c_view - foreign - [c_view]->[view]:[id]
...
Column List:
	...
	id - int,req - unique identifier for this block statement
	block - int,req - what block this statement is in
	order - int,req - what order a statement executes in
	stmt_type - str,req - what type of stmt this is; eg: assignment, condit, call, loop, dml, return
	dest_var - int - if the stmt type is 'assign', this var is where the expression result goes
	sproc - enum - if expr_type is 'sproc', then this is the standard built-in procedure to call
	uproc - int - if expr_type is 'uproc', then this is the user-defined stored function (block) to call
	c_block - int - if the stmt type starts an unnamed block context, this is the child block id
...
Index List:
	...
	ak_order - unique - [block,order]
	fk_block - foreign - [block]->[block]:[id]
	fk_dest_var - foreign - [dest_var]->[block_var]:[id]
	fk_sproc - foreign - [sproc]->[cct_standard_proc]:[code]
	fk_uproc - foreign - [uproc]->[block]:[id]
	fk_c_block - foreign - [c_block]->[block]:[id]
...
Column List:
	...
	id - int,req - unique identifier for this block statement sub-expression
	stmt - int,req - unique identifier for block statement this expression is part of
	p_expr - int - parent expression of this one, if any; empty if this is the root expression
	p_expr_order - int,req - order of this expression among its siblings; 1 if no siblings
	expr_type - enum,req - what type of expression this is; eg: literal, variable, column, function
	lit_val - str - if expr_type is 'lit', then store the literal value here
	src_var - int - if expr_type is 'var', then use this containing block variable
	sfunc - enum - if expr_type is 'sfunc', then put the standard built-in function name/code here
	ufunc - int - if expr_type is 'ufunc', then put the user-defined stored function (block) id here
...
Index List:
	...
	primary - unique - [id]
	ak_order - unique - [stmt,p_expr,p_expr_order]
	fk_stmt - foreign - [stmt]->[block_stmt]:[id]
	fk_expr_type - foreign - [c_expr_type]->[cct_basic_expr_type]:[code]
	fk_src_var - foreign - [src_var]->[block_var]:[id]
	fk_sfunc - foreign - [sfunc]->[cct_standard_func]:[code]
	fk_ufunc - foreign - [ufunc]->[block]:[id]
...
=head1 APPLICATIONS, COMMANDS AND RESULTS
...
To be added: application, command, command_var, result.

=head1 SEE ALSO

perl(1), SQL::ObjectModel, SQL::ObjectModel::XMLSchema, Rosetta,
Rosetta::Framework.
