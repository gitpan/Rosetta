...
my $DBG_GAP_ATTR_ID   = 'id'; # attribute name to use for the node id
...
sub _serialize_as_perl {
	...
	$pad ||= '';
	my $padc = $ind ? "" : "$pad\t";
	my %key_order = ( $DBG_GAP_NODE_TYPE => 3, $DBG_GAP_ATTRS => 2, $DBG_GAP_CHILDREN => 1, );
	return( join( '', 
		ref($input) eq 'HASH' ? 
			( ($is_val ? '' : $pad).'{ '.(%{$input}?"\n":''), ( map { 
...
sub _get_all_properties {
	...
	$dump{$DBG_GAP_ATTRS} = {
		$DBG_GAP_ATTR_ID => $node->{$NPROP_NODE_ID},
		%{$node->{$NPROP_AT_LITERALS}},
		%{$node->{$NPROP_AT_ENUMS}},
...
distribution when ready.  You could also learn something from the code samples 
inside other modules which sub-class this one.
...
also lacking a description in DataDictionary.pod.  In a near-term update,
probably within 2 releases, several Node Types will be altered or merged or
added, and DataDictionary.pod will be updated accordingly.
