...
The SQL::SyntaxModel language does not require you to explicitely run a
...
logic control operations or value assignment operations.
	...
	COMMAND - a call to a SQL::SyntaxModel recognized command, with a Node as its argument
...
"db" can mean either "database" or "data source" or "catalog link"; "ds" may be used instead.
	...
	DB_ATTACH    - opens a link or connection to a database instance, or attaches it
	DB_DETACH    - closes a link or connection to a database instance, or detaches it
...
It is here to for "schema" Nodes to reference as their owner.  A separate
...
Example List:
	...
	|id|contex|type    |sc|name|wr|
	|--|------|--------|--|----|--|
	| 1|APPLIC|MULTIPLE|  |    |1 | - user - joins the "user_auth" and "user_profile" tables
	| 2|APPLIC|MULTIPLE|  |    |0 | - person_with_parents - joins the "person" table with itself twice
	| 3|APPLIC|SINGLE  |  |    |0 | - user_theme - shows how many users use each theme pref value
	| 4|APPLIC|MATCH   |  |    |1 | - person - simply the "person" table
...
Attribute List:
	...
	domain - N.domain - MA - the user-customized data type for this column
	src_col - N.view_src_col - use with view_type=MATCH; the single source table/view column to match
...
Example List:
	...
	| 2| 1|login_name   |23| - who user authenticates into app as
	...
	|17| 2|self_name    |24| - actual name of the person they are called by
	|18| 2|father_id    | 9| - person_id of this person's father, if known
	|19| 2|father_name  |24| - actual name of the person's father
	|20| 2|mother_id    | 9| - person_id of this person's mother, if known
	|21| 2|mother_name  |24| - actual name of the person's mother
...
Attribute List:
	...
	set_view_col - N.view_col - MC(view_part=SET) - this is the column we are setting the value of in insert or update op
...
Example List:
	...
	|id|cl|vw|part  |pe|type |lit  |blv|cmv     |src|mtc|sf    |uf|
	|--|--|--|------|--|-----|-----|---|--------|---|---|------|--|
...
executing a DML operation.  Often, most details for a statement are in child 
expression Nodes.
...
Attribute List:
	...
	command - E.command_type - MC(stmt_type=COMMAND) - what SQL::SyntaxModel recognized command to run - MOCKUP
	command_arg - L.uint - MC(stmt_type=COMMAND) - the Node ID of the argument to the above command - MOCKUP
	...
	call_sproc - E.standard_proc - if expr_type is 'sproc', then this is the standard built-in procedure to call
	call_uproc - N.routine - if expr_type is 'uproc', then this is the user-defined stored function (routine) to call
...
Attribute List:
	...
	lit_val - L.misc - MC(expr_type=LIT) - the literal value of this expression
	src_arg - N.routine_arg - MC(expr_type=ARG) - the current routine argument to read the value of
	src_var - N.routine_var - MC(expr_type=VAR) - the current routine variable to read the value of
	call_sfunc - E.standard_func - MC(expr_type=SFUNC) - the standard built-in function we will invoke
	call_ufunc - N.routine - MC(expr_type=UFUNC) - the user-defined stored function we are calling
...
=head2 database_product

This type of Node describes a distinct database product that someone can use to
implement a database in, such as Oracle 9i or PostgreSQL 7.4.  The attributes
in this Node mainly just have meaning to external code that mediates between a
SQL::SyntaxModel and the native drivers for database products; typically they
are used to map a specific native driver to use, and/or a specific SQL
generator to use for the mediation.  If you are using multiple versions of the 
same database product line for different database instances, you should have a 
separate one of these Nodes for each, as the different versions may not support 
the same features or interface.
...
Attribute List:
	...
	product - N.database_product - MA - what product implements this; determines SQL dialect to use
...
instead.  I<Details to be modified later.>

Attribute List:

	id - NODE_ID - MA - unique identifier for each Node within this Node Type
	unrealized - N.catalog_link - MA - the unrealized catalog link that we are 'implementing'
	...
	local_dsn - L.cstr - locally known driver-specific name for the catalog link
...
Attribute List:
	...
	id - NODE_ID - MA - unique identifier for each Node within this Node Type
	product - N.database_product - if set, only use with this product; if not, use with all prods
