...
These commands apply to whole databases and most of them are not dependant on a 
pre-existing context; one or two may make or depend on a db Connection context.
...
These DDL commands can only be used within the context of a database
Connection, as usually is created by a DB_OPEN command, and destroyed by a
DB_CLOSE.
...
These commands may or may not be easily implementable by routines.
...
These DML commands can only be used within the context of a database
Connection.  They may also be ignored in favor of the "routine" alternatives.
...
containing all of the longer-name objects, unless it was explicitely told
