...
To become "SQL-SyntaxModel" or similar:
	...
	SQL::SyntaxModel (An abstract syntax tree for all types of SQL)
		SQL::SyntaxModel::Container (a context for Nodes, one of these per parent)
		SQL::SyntaxModel::Node (a semi-atomic model component)
...
Rosetta::Engine subclass which the normal Engine modules that use them (or
"it" if they're combined) would subclass; that may work better than the normals
subclassing Rosetta::Engine and using DBIWrapper or SQLBuilder.
...
supported or not is not applicable.  (However, 2.8.13 is the newest right now.)
...
PostgreSQL 7.4.1 databases, with 7.3.6 support likely present but deprecated. 
