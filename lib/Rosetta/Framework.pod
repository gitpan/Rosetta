...
including addition of more modules or removal of some existing ones.  Also, not 
all of these exist yet, as they are being implemented over time.

=head2 In distribution "Rosetta"

Note that the core modules which are scheduled to be in the "Rosetta"
distribution I<may> be split into more distributions in order to help fuel a
public perception that they can indeed be used separately from each other, and
the fact that users do get a high level of customization.  If this goes
through, then we will probably split the core into 3 distributions: "Rosetta"
will contain all the main documentation and it will only have documentation (as
it mostly did as of release 0.09); "Rosetta-Schema" will have just the
"Rosetta::Schema/::*" modules and can be used as a fully-parsed replacement for
SQL strings; "Rosetta-Engine" will have the "Rosetta::Engine/::*" and the
"Rosetta::Locale/::*" modules, and be sort of a runtime environment, which is a
thin interface on top of the driver modules.  Whether or not this split will be 
done is largely up to what would benefit the developer or user community more.  

Note also that if this split happens then the placeholder "Rosetta" module
won't even provide a shim interface for Rosetta::Engine->new().  Not that it is
necessarily a good practice in any event.
