...
(SQL::SyntaxModel), "Engine", "Driver", "Proxy" (Server/Client), "Validator",
"Emulator", and others.
...
including addition of more modules or removal of some existing ones.  Also, as
of the Rosetta distribution v0.23, the 'Schema Modules' section has been
removed from Framework.pod and its contents put in Language.pod.
...
database is embedded into the driver, we simply use the latest version
available at the time of each driver update, so mention of version ranges being
supported or not is not applicable.
...
PostgreSQL 7.4 databases, with 7.3 support likely present but deprecated. 
