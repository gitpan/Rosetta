...
embed both main versions, v3.0.7 and v2.8.15 (both of which are declared
stable); the 2.x support is deprecated.

=item

B<Rosetta::Engine::MySQL> - This class implements an Engine for talking to
MySQL databases.  It is recommended for use only with MySQL versions 4.1.5 and
greater, including v5.0.x and v5.1.x.  Support for MySQL versions 4.0.21 and
3.23.58 is present but deprecated.  v3.22.x and earlier will not be supported at
...
8.0.0 and greater.  Support for PostgreSQL versions 7.4.5 and 7.3.7 and 7.2.5
