...
=head2 Content of a simple CGI script for implementing a web survey:
	...
	sub script_make_screen {
		...
		my $dbh = $engine->execute_command( {
			'type' => 'database_open',
			'args' => $prefs->{database_open_args}, # includes what driver to use
		} );
		if( $dbh->is_error() ) {
...
=head2 In distribution "Rosetta"
	...
	Rosetta::Engine::Database (represents a connection)
...
=head2 In distribution "Rosetta-Drivers" or a multiplicity of similarly named
	...
	Rosetta::Driver::DB2 (interface to IBM DB2 db)
...
B<Rosetta::Engine::Database> - This Engine class represents a connection
...
databases.  It is intended to be used only with MySQL versions 4.0.12 and
...
perl(1), Rosetta::API_C, Rosetta::SimilarModules, Rosetta, Rosetta::Schema::DataType,
Rosetta::Schema::Table, Rosetta::Schema::View.
