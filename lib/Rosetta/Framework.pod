...
categories, which can be called: "Locale" (Locale::KeyedText), "Schema"
(SQL::SyntaxModel), "Interface", "Engine", "Proxy" (Server/Client), "Validator",
"Emulator", and others.
...
This briefly illustrates the relationship of the module groups:
	    ...
	    called by |   uses   used by \
	              |                   \
	      invokes |                    \
	          Interface--------------Schema;Locale (both used by other 7 groups)
	    called by | \ uses   used by /  |
	              |  \              /   |
	      invokes/|   \            /   /
	            //| Emulator------/   /
...
The "Schema" and "Interface" modules are collectively referred to as the "Rosetta
...
run your application on; they are "action" modules.  They take Interface/Schema
...
the results into other Interface/Schema objects to return as output.  The Engine
...
from known dialects into Interface/Schema objects
...
The C version of Rosetta will probably have the "Schema" (SQL::SyntaxModel) and
...
dependant on but which are also useful by themselves.  See below for details.
The split will not be done immediately, although the modules have been renamed
already.  Most of these modules don't exist yet, but are being planned to
exist, as they are implemented over time.
...
To become "SQL-SyntaxModel" or similar:

	SQL::SyntaxModel (An abstract syntax tree for all types of SQL)
		SQL::SyntaxModel::_::Container (a context for Nodes, one of these per parent)
		SQL::SyntaxModel::_::Node (a semi-atomic model component)
	SQL::SyntaxModel::L::en (Localization of SQL::SyntaxModel for English)
	SQL::SyntaxModel::L::fr (Localization of SQL::SyntaxModel for French)
	SQL::SyntaxModel::L::* ... like German, Spanish, Dutch, Chinese, etc
...
To become "SQL-SyntaxModel-ByTree" or similar:
...
To become "SQL-SyntaxModel-SkipID" or similar:
...
To stay "Rosetta":
	...
	Rosetta (Framework for RDBMS-generic apps and schemas)
		Rosetta::_::Interface (defines the whole Rosetta public API, is largely a shim)
		Rosetta::_::Engine (common base class for Engine modules that implement the Rosetta API)
	...
	Rosetta::Engine::Null (an engine with no functionality, for testing core)

=head2 In distribution "Rosetta-Engines" or a multiplicity of similarly named
...
"http://freshmeat.net/articles/view/305/" and "http://freshmeat.net/articles/view/307/">
	...
	Rosetta::Engine::SQLite (binding to public domain SQLite embedded db; default RDBMS for Rosetta)
	Rosetta::Engine::Oracle (interface to Oracle db 8, 9)
	Rosetta::Engine::Sybase (interface to Sybase db)
	Rosetta::Engine::PostgreSQL (interface to PostgreSQL db 7.4.x, 7.3.x)
	Rosetta::Engine::MySQL (interface/binding to MySQL cli-serv/embed db 4.x, 5.x, 3.23.x)
	...
	Rosetta::Engine::OpenBase (interface to OpenBase db)
	...
	Rosetta::Engine::Common::RWViews (convert IUD against views into IUD against tables)
	Rosetta::Engine::Common::SQLBuilder (generate ANSI/ISO SQL-1999 and other SQL variants)
	Rosetta::Engine::Common::DBIWrapper (helps engines implemented using DBI/DBD::*)
	Rosetta::Engine::Common::* ... other code for sharing between multiple engines

Note that DBIWrapper and/or SQLBuilder may get implemented as a
Rosetta::_::Engine subclass which the normal Engine modules that use them (or
"it" if they're combined) would subclass; that may work better than the normals
subclassing Rosetta::_::Engine and using DBIWrapper or SQLBuilder.
...
=head2 In distribution "Rosetta-Emulators"
	...
	Rosetta::Emulator::Common::SQLParser (parse ANSI/ISO SQL-1999 and other SQL variants)
...
release of the Rosetta distribution.  Some details you see here shouldn't be
considered accurate.  One portion that was here as late as release 0.22,
'Schema Modules', has been moved to Language.pod.
...
B<Rosetta::_::Interface> - This Interface class is inherited by all other Interface
...
B<Rosetta::_::Command> - This Interface class describes an action that needs
to be done against a database; its objects encapsulate
SQL::SyntaxModel::Command objects and also encapsulates the resolved bind vars,
...
SQL::SyntaxModel::_::Command.>

=item

B<Rosetta::_::Result> - This Interface class is inherited by all Interface
...
B<Rosetta::_::Connection> - This Interface class represents a connection
...
B<Rosetta::_::Cursor> - This Interface class represents a cursor over a
...
supported or not is not applicable.  (However, 2.8.12 is the newest right now.)

=item

B<Rosetta::Engine::Oracle> - This class implements an engine for talking to
Oracle 8.x, 9.x, 10.x databases.  Note that Oracle 9 is the first version of the
...
PostgreSQL 7.4.1 databases, with 7.3.5 support likely present but deprecated. 
Versions 7.2 and earlier won't be supported without demand.

=item

B<Rosetta::Engine::MySQL> - This class implements an engine for talking to MySQL
databases.  It is intended to be used only with MySQL versions 4.0.18 and
later, with version 4.1.1 or greater being strongly recommended.  That said,
...
the 4.0.x series is production quality right now (2003.03.15).  Support for
3.23.x will be present in the short term, but it is deprecated.  3.22.x and
earlier will not be supported at all, without great demand.

=item

B<Rosetta::Engine::Common::RWViews> - This is a utility class containing common
...
B<Rosetta::Engine::Common::SQLBuilder> - This is a utility class containing
...
compliant with the ANSI/ISO SQL-1999 standard.

=item

B<Rosetta::Engine::Common::DBIWrapper> - This is a utility class containing
common functionality for multiple Engines.  Given that the pure perl versions
...
functionality for DBI that makes it easier for Rosetta engines to use it.
...
in Framework.pod since the first release of the Rosetta distribution; it was
moved down here during release 0.24.  Some details you see here shouldn't be
considered accurate; the new SYNOPSIS in Rosetta.pm is loosely derived from
this, and that is up to date.
...
=head2 Content of a simple CGI script for implementing a web survey:
	...
	sub script_make_screen {
		...
		eval {
			require Rosetta; # also compiles SQL::SyntaxModel
		};
		if( $@ ) {
	...
	sub script_while_opened {
		...
		my $dd_table = SQL::SyntaxModel->new_table( 'survey_data' );
	...
	sub script_to_fillin {
		...
		my $dd_view = SQL::SyntaxModel->new_view( $dd_table );
...
perl(1), Rosetta, SQL::SyntaxModel, Locale::KeyedText, DBI.
