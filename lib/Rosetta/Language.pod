...
Rosetta::Model contains SQL schemas and queries, represented as a tree of
...
XML schema.  See L<Rosetta::Model> for more details.
...
This document, Rosetta::Language, is meant to say what all the 'types'
of Nodes are, and what attributes and relationships are allowed for each.
It is meant to say what 'grammar' for Rosetta::Model's 'language' is, or what
'schema' it accepts.
...
Rosetta::Model mainly follows its own path regarding its language structure;
...
Rosetta::Model aims to be a "90%+ solution", natively supporting what I
...
Rosetta::Model is designed largely in the hindsight of the ANSI/ISO SQL:1999
...
versions talk about the same features using different words, Rosetta::Model
...
Generally speaking, Rosetta::Model holds to a purely relational database
...
For such situations where you want to use Rosetta::Model to mediate your
access to a database, and it doesn't natively support a database product
feature that you want to use, Rosetta::Model does provide a "back door" where
you have un-mediated access to the database.  Speaking more accurately,
Rosetta::Model can store fragments of any raw SQL that you specify, to be
...
that, then Rosetta::Model lets you store alternative SQL fragments that are
...
Rosetta::Model manages SQL concepts that we arrange into 5 main categories
...
one or more externally (to Rosetta::Model and your application core)
...
The last category, 'circumventions', is used to implement Rosetta::Model's
"circumvention" features, to access concepts that a specific database
product understands but nothing in the Rosetta::Model code does.
...
level Rosetta::Model language concepts; each distinct concept is represented
...
single Rosetta::Model Node Type.  Each Node Type is explained in further
detail below.
...
The Rosetta::Model language is designed with an embedded SQL paradigm in
...
The Rosetta::Model language is designed so that you would do B<all> types of
...
procedure that connect to a second database.  In fact, the Rosetta::Model
...
The Rosetta::Model language takes a broad definition as to what constitutes a
...
routine arguments and/or has an explicit return value.  The Rosetta::Model
...
behalf, and that take Rosetta::Model language constructs as input, will be
...
latter would take a Rosetta::Model "routine" Node (or an object wrapping one
...
The Rosetta::Model language does not require you to explicitly run a
...
There are 4 major Rosetta::Model Node attribute types.  Each major type may
...
Currently, Rosetta::Model recognizes these explicit literal attribute types:
...
Here is a compact list of the enumerated attribute types that Rosetta::Model
...
Here is a compact list of the Node Types that Rosetta::Model knows about;
each one corresponds to a Rosetta::Model concept.  A Node Ref attribute of a
certain type will reference another Node of that Node Type.  For full
details on each one, please see L<Rosetta::NodeTypes>.
...
high-level Rosetta::Model concepts, back when each concept was planned to be
...
B<Rosetta::Model::Command> - This Model class describes an action that needs
...
B<Rosetta::Model::DataType> - This Model class describes a simple data type,
...
B<Rosetta::Model::Table> - This Model class describes a single database
...
database data.  This class can generate Rosetta::Model::Command objects
having types of: 'table_verify', 'table_create', 'table_alter',
'table_destroy'.

B<Rosetta::Model::View> - This Model class describes a single database view,
...
Rosetta::Model::Command objects having types of: 'data_select',
'data_insert', 'data_update', 'data_delete', 'data_lock', 'data_unlock',
'view_verify', 'view_create', 'view_alter', 'view_destroy'.

=head1 SEE ALSO

L<Rosetta::Model> and the various other modules mentioned in its SEE ALSO.
