...
are part of Rosetta::Model's language or grammar.  You should read the
summary introduction in L<Rosetta::Language> before this document.

=head1 ENUMERATED TYPES

This document details the enumerated types that Rosetta::Model knows about.
...
have an implicit container_type of SCALAR.
    ...
    ROS_M_NODE      - for client code only; stores a Rosetta::Model::Node object
    ROS_M_NODE_LIST - for client code only; stores a list of Rosetta::Model::Node objects

    If either of ROS_M_NODE[|_LIST] appear with a ":foo" appended, such as
    "ROS_M_NODE:foo", that means only a "foo" Node (or list thereof) is taken,
...
Rosetta::Model virtual database language.  The action types include:
...
routine_expr, view_expr] ROS M Nodes can have a 'standard_routine' attribute
...
client-side routine, usually with 'routine_stmt' Nodes.
    ...
    CATALOG_LIST()
        ...
        effects:
            - A new 'catalog' Node is added to the Rosetta::Model::Container for
        ...
        returns:
            - ROS_M_NODE_LIST:catalog_link - The newly added cli Nodes.
        throws:
            - ROSMX_NO_ENVI_LOAD_FAILED, ROSMX_ENVI_EXEC_FAILED,
            ROSMX_NO_CONN_SERVER_ABSENT, ROSMX_NO_CONN_BAD_AUTH,
            ROSMX_NO_CONN_ACTIVE_LOST
    ...
    CATALOG_INFO()
        ...
        args:
            - LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to scan.
        ...
        returns:
            - ROS_M_NODE:catalog_link - The same cli that LINK_BP has.
        throws:
            - ROSMX_NO_ENVI_LOAD_FAILED, ROSMX_ENVI_EXEC_FAILED,
            ROSMX_NO_CONN_SERVER_ABSENT, ROSMX_NO_CONN_BAD_AUTH,
            ROSMX_NO_CONN_ACTIVE_LOST
    ...
    CATALOG_VERIFY()
        purpose:
            ...
            are made to the Rosetta::Model::Container; existing Nodes therein are
            compared to instead.
        args:
            - LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to scan.
        ...
        returns:
            ...
            itself are more ambiguous or less detailed than the ROS M model that
    ...
    CATALOG_CREATE()
        ...
        args:
            - LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to create.
    ...
    CATALOG_DELETE()
        ...
        args:
            - LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to delete.

    CATALOG_CLONE()
        purpose:
            - Makes a clone of a db instance in another storage loc.
        args:
            - SOURCE_LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to copy from.
            - DEST_LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to copy to / create.

    CATALOG_MOVE()
        purpose:
            - Moves a db inst from one storage location to another.
        args:
            - SOURCE_LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog location to move from.
            - DEST_LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog location to move to.
...
routine, client or database side, usually with 'routine_stmt' Nodes.
    ...
    CATALOG_OPEN()
        purpose:
            ...
            - Note that it is the routine_var or routine_arg declaration ROS M
    ...
    CATALOG_PING()
        ...
        returns:
            ...
            false if not.
            - Since we are explicitly testing it, no ROSMX_NO_CONN_ACTIVE_LOST
    ...
    CATALOG_ATTACH()
        ...
        args:
            - LINK_BP(1) - ROS_M_NODE:catalog_link - The catalog to connect to.
        throws:
            - ROSMX_NO_CONN_ACTIVE_LOST
...
database side, usually with 'routine_stmt' Nodes.
    ...
    SELECT()
        ...
        context:
            - CONN_CX - CONN - An open database connection to provide our action context.
        args:
            - SELECT_DEFN(1) - ROS_M_NODE:view - The view that defines this select action.
    ...
    INSERT()
        ...
        context:
            - CONN_CX - CONN - An open database connection to provide our action context.
        args:
            - INSERT_DEFN(1) - ROS_M_NODE:view - The view that defines this insert action.
    ...
    UPDATE()
        ...
        context:
            - CONN_CX - CONN - An open database connection to provide our action context.
        args:
            - UPDATE_DEFN(1) - ROS_M_NODE:view - The view that defines this update action.
    ...
    DELETE()
        ...
        context:
            - CONN_CX - CONN - An open database connection to provide our action context.
        args:
            - DELETE_DEFN(1) - ROS_M_NODE:view - The view that defines this delete action.
...
view, client or db side, usually with 'routine_expr' or 'view_expr' Nodes.
    ...
    CAST()
        ...
        args:
            - CAST_TARGET(1) - ROS_M_NODE:scalar_data_type,scalar_domain - The data type to cast the operand as.
...
in the "from" part.
    ...
    Note that Rosetta::Model may need more metadata to represent parts of a GROUP BY
    ...
    more powerful than that.  Rosetta::Model may need more metadata to do this right.
...
These represent the basic types that a routine can be, since Rosetta::Model
